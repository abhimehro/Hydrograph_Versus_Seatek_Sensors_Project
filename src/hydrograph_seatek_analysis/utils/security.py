...
def sanitize_filename(filename: str, max_length: int = 200) -> str:
    ...
    contains_non_ascii = any(ord(char) > 127 for char in filename)
    ...
    # Keep distinct Unicode names from colliding after their characters are replaced.
    # The digest is ASCII and stable for the original input.
    suffix = ""
    ...
    if suffix:
        return (sanitized[: max(0, max_length - len(suffix))] + suffix)[:max_length]
