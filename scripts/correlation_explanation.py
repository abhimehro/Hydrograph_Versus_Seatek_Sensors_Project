...
def explain_correlation_example():
    ...
    print("----------------")
    print(
        "- The correlation coefficient of {:.2f} indicates a {} relationship".format(
            correlation, "positive" if correlation > 0 else "negative"
        )
    )
    print(
        "- This means that as hydrograph discharge increases, sensor readings tend to",
        "increase" if correlation > 0 else "decrease",
    )
    print(
        f"- The strength of this relationship is captured in the magnitude: {abs(correlation):.2f}"
    )
