...
def process_rm_data(rm_data, rm, year, sensor):
    ...
    plt.tight_layout(rect=[0, 0, 0.85, 1])  # Adjust right side for legend without excessive white space
    ...
    # Calculate and display correlation only once
    valid_mask = mask_hydro & mask_sensor
    
    if sum(valid_mask) > 1:
        correlation = year_data["Hydrograph_(Lagged)"][valid_mask].corr(year_data[sensor][valid_mask])
        
        # Display correlation box in a better position and only once
        plt.figtext(0.92, 0.05, f'Correlation: {correlation:.2f}',
                    fontsize=9,
    ...
    # Add min/max points for both hydrograph and sensor data
    if any(mask_hydro):
    ...
    # Move legend closer to plot without overlap or excessive white space
    ax1.legend(handles=legend_elements,
               bbox_to_anchor=(1.05, 1),  # Moved closer to plot but still outside
               loc="upper left",
               fontsize=10,
               ncol=1,
               framealpha=0.9)
