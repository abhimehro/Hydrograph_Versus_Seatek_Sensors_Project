...
Functions:
    ...
    Main():
        Main function that loads the summary data, iterates through each row, and generates plots for each river mile
        (RM) and year range specified in the summary.
...
def create_chart ( data , rm_value , year , output_dir ) :
	plt.figure ( figsize = (12 , 10) )
	plt.plot ( data [ "Time (Seconds)" ] , data [ "Hydrograph" ] , label = "Hydrograph" , color = "blue" )
	plt.plot ( data [ "Time (Seconds)" ] , data [ "Sensor_Value" ] , label = "Sensor" , color = "orange" )
	plt.title ( f"RM {rm_value} Year {year}" )
	plt.xlabel ( "Time (Seconds)" )
	plt.ylabel ( "Value" )
	plt.legend ( )
	plt.savefig ( f"{output_dir}/RM_{rm_value}_Year_{year}.png" , dpi = 300 )
	plt.close ( )
