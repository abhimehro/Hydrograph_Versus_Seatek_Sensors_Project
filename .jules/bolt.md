...
## 2024-05-14 - Prevent Unnecessary Series Allocation Before Pandas Native Aggregations
**Learning:** Pandas native aggregation functions (like `.min()`, `.max()`) implicitly skip NaN values by default (`skipna=True`). Calling `.dropna()` before an aggregation creates a redundant intermediate Pandas Series in memory, increasing allocation overhead and negatively impacting performance without altering the mathematical result.
**Action:** Remove `.dropna()` before calling aggregate functions like `.max()` or `.min()` on Pandas Series to save memory and execution time. Ensure this pattern is maintained across the codebase, particularly inside frequently executed plotting and mathematical operations.
