...
## 2024-06-11 - Replace df[cols].isna().sum() with numpy comprehension
**Learning:** Using `df[cols].isna().sum()` to count missing values across multiple columns evaluates a Pandas Series by creating an intermediate boolean DataFrame mask and upcasting the values implicitly. This incurs significant Pandas DataFrame overhead. By rewriting this using a dictionary comprehension over the underlying numpy arrays (`pd.Series({col: np.count_nonzero(pd.isna(df[col].values)) for col in cols})`), it bypasses Pandas DataFrame `__getitem__` allocation overhead and implicitly avoids upcasting boolean values to integers, significantly improving execution speed.
**Action:** Replace `df[cols].isna().sum()` with `pd.Series({col: np.count_nonzero(pd.isna(df[col].values)) for col in cols})` and ensure `import numpy as np` is included.
