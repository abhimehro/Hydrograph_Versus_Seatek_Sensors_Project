...
**Prevention:** When implementing file validations using `pathlib.Path`, explicitly reject symbolic links using `file_path.is_symlink()` before validating size or reading the file to prevent arbitrary file read or validation bypass attacks.
...
## 2026-04-16 - Legacy Test Data Inspection and Validation Bypassed DoS File Size Guard
**Vulnerability:** Memory exhaustion (DoS) vulnerability in `tests/data_processing/data_validation_test.py` and `tests/data_processing/data_inspection_test.py` where `pd.ExcelFile()` was called without prior file size validation, exposing the test utilities to processing excessively large malicious or malformed test files.
**Learning:** Security validations (e.g., file size checks) must be consistently applied across the entire codebase, including auxiliary testing scripts and utilities like data inspectors and validators, as malicious actors can exploit secondary entry points.
**Prevention:** Always implement `validate_file_size(file_path, max_size)` from `utils.security` before any instantiation of `pd.ExcelFile()` or `pd.read_excel()`, even in test scripts like `data_validation_test.py` or `data_inspection_test.py`.
