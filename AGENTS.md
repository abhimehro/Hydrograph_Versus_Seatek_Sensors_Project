...
- Default `python3 -m pytest tests/` already skips legacy trees via `norecursedirs` in `pyproject.toml` (`tests/data_processing`, `tests/utils`, `tests/visualization`). That collects the maintained root `tests/test_*.py` suite. Note: modern chart tests under `tests/visualization/` are also excluded by that setting — re-enable deliberately if desired. `tests/enhanced_test_suite.py` and the notebook still reference the removed `src.data_processing` / `src.visualization` paths.
...
- PR and `main` CI runs `pytest` + `flake8` + `mypy`. See `.github/workflows/python-tests.yml`.
