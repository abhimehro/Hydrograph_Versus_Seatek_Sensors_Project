...
FORMATTERS = {
    "BBCodeFormatter": (
        ...
        ("bbcode", "bb"),
        (),
        "Format tokens with BBcodes. These formatting codes are used by many bulletin boards, "
        "so you can highlight your sourcecode with pygments before posting it there.",
    ),
    "BmpImageFormatter": (
        ...
        ("bmp", "bitmap"),
        ("*.bmp",),
        "Create a bitmap image from source code. This uses the Python Imaging Library to generate a pixmap "
    ...
    "GifImageFormatter": (
        ...
        ("gif",),
        ("*.gif",),
        "Create a GIF image from source code. This uses the Python Imaging Library to generate a pixmap from "
        "the source code.",
    ),
    "GroffFormatter": (
    ...
    "HtmlFormatter": (
        ...
        ("html",),
        ("*.html", "*.htm"),
        "Format tokens as HTML 4 ``<span>`` tags within a ``<pre>`` tag, wrapped in a ``<div>`` tag. The "
        "``<div>``'s CSS class can be set by the `cssclass` option.",
    ),
    "IRCFormatter": (
