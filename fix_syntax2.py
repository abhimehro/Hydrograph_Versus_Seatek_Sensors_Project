...
def fix_file(filepath):
    ...
    content = content.replace("pd.NA if pd.api.types.is_object_dtype(merged[sensor]) else np.nan\\n", "pd.NA if pd.api.types.is_object_dtype(merged[sensor]) else np.nan\n")
    content = content.replace("pd.NA if pd.api.types.is_object_dtype(merged['Hydrograph (Lagged)']) else np.nan\\n", "pd.NA if pd.api.types.is_object_dtype(merged['Hydrograph (Lagged)']) else np.nan\n")
